...
import {
  ...
  TInitialState,
  TWsConnectionStart,
} from "../services/wsRedux/action-types";
...
type TList = {
  ws: TInitialState;
  wsConnection: () => TWsConnectionStart;
};
export const OrderId: FC<TOrderId> = ({ id }) => {
  const history = useHistory();
  const dispatch = useDispatch();
  const { data } = useSelector((state) => state);
  const { feed, ordersHistory } = useSelector((state) => state);
  let list: TList = { ws: ordersHistory, wsConnection: wsConnectionStartHistory };
  history.location.pathname.indexOf("feed") !== -1 && (list = { ws: feed, wsConnection: wsConnectionStartFeed });
